...
-- local Postgres docker image — only the upstream source repo ships it as
-- extension-control files. Embedding the canonical SQL inline gives identical
-- runtime semantics across local + Cloud and avoids depending on the image
-- shipping an extension binary.
--
...
-- under a `tests` / `test_overrides` schema, only callable explicitly, and add
-- no policies.
-- ============================================================================
